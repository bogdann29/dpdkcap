...
#define cp_skip_whitespace(c, e)         \
	while (c < e && cp_is_whitespace(c)) \
	{                                    \
		c++;                             \
	}
#define cp_skip_eol(c, e)         \
	while (c < e && cp_is_eol(c)) \
	{                             \
		c++;                      \
	}
#define cp_skip_sep(c, e)         \
	while (c < e && cp_is_sep(c)) \
	{                             \
		c++;                      \
	}

#define cp_find_eol(c, e)          \
	while (c < e && !cp_is_eol(c)) \
	{                              \
		c++;                       \
	}
#define cp_find_eot(c, e)                                                \
	while (c < e && !(cp_is_eol(c) || cp_is_sep(c) || cp_is_comment(c))) \
	{                                                                    \
		c++;                                                             \
	}
#define cp_find_eotm(c, e)                               \
	while (c < e && !(cp_is_eol(c) || cp_is_comment(c))) \
	{                                                    \
		c++;                                             \
	}
