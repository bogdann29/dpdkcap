...
static struct argp_option options[] = {
	...
	{"portmask", 'p', "PORTMASK", 0, "Ethernet ports mask (default: 0x1).",
	 0},
	{"snaplen", 's', "LENGTH", 0,
	 "Snap the capture to snaplen bytes "
	 "(default: 65535).",
	 0},
	{"logs", 700, "FILE", 0,
	 "Writes the logs into FILE instead of "
	 "stderr.",
	 0},
...
/**
 * @struct arguments
 * @brief Struct to hold program arguments
 */
struct arguments
{
	char *args[2];											   /**< Command line arguments */
	char output_file_template[DPDKCAP_OUTPUT_FILENAME_LENGTH]; /**< Output file template */
	uint64_t portmask;										   /**< Port mask */
	int statistics;											   /**< Flag for enabling statistics */
	unsigned long nb_mbufs;									   /**< Number of mbufs */
	char *num_rx_desc_str_matrix;							   /**< Number of Rx descriptors */
	unsigned long per_port_c_cores;							   /**< Number of cores per port */
	unsigned long num_w_cores;								   /**< Number of worker cores */
	int no_compression;										   /**< Flag for disabling compression */
 ...
 *
 * Parses the given command line option and updates the program arguments
 *
...
static const struct rte_eth_conf port_conf_default = {
	.rxmode = {
		.mq_mode = RTE_ETH_MQ_RX_NONE,		   /**< Multiqueue mode */
		.max_lro_pkt_size = RTE_ETHER_MAX_LEN, /**< Maximum LRO packet size */
	}};

/**
 * @brief Initializes a given port using global settings and with the RX buffers
 * coming from the mbuf_pool passed as a parameter.
 *
 * @param port The port to initialize
 * @param rx_rings The number of RX rings
 * @param num_rxdesc The number of RX descriptors
 * @param mbuf_pool The memory pool for RX buffers
 * @return 0 on success, -EINVAL on error
 */
...
/**
 * @brief  main function, which does initialization and calls the per-lcore functions.
 */
int main(int argc, char *argv[])
{
	...
	for (port_index = 0; port_index < nb_ports; port_index++)
	{
		...
		write_ring = rte_ring_create(strbuf,
									 rte_align32pow2(arguments.nb_mbufs),
									 socket_id, 0);
		...
		for (j = 0; j < arguments.num_w_cores; j++)
		{
			...
			config->stop_condition =
				get_stopper_for_socket(socket_id) || (arguments.total_files_size_limit != 0 && arguments.total_files_size_limit < config->stats->bytes);
			config->taskdir =
			...
			// Launch writing core
			if (rte_eal_remote_launch((lcore_function_t *)write_core, config,
									  core_id) < 0)
				rte_exit(EXIT_FAILURE,
		...
		for (j = 0; j < arguments.per_port_c_cores; j++)
		{
			...
			config->queue = j;
			// Launch capture core
			if (rte_eal_remote_launch((lcore_function_t *)capture_core, config,
									  core_id) < 0)
				rte_exit(EXIT_FAILURE,
