...
/**
 * @brief Close and free a pcap file
 *
 * @param file FILE pointer to the opened file
...
/**
 * @brief Allocates a new lzowrite_buffer from the given file
 *
 * @param output_file Output file name
 * @return Pointer to the allocated lzowrite_buffer, or NULL on failure
 */
...
/**
 * @brief Free a lzowrite_buffer
 *
 * @param buffer Pointer to the lzowrite_buffer to be freed
 * @return 0 on success, nonzero value on failure
 */
	...
	for (;;)
	{
			 ...
			 task_idx++)
		{
			...
			for (i = 0; i < to_write; i++)
			{
				...
				if (task->output_buffer && task->output_rotate_size && task->output_size >= task->output_rotate_size)
				{
					task->output_count++;
					file_close_func(task->output_buffer);
					task->output_buffer = NULL;
				...
				if (task->output_buffer == NULL)
				{
					...
					// Change file name
					format_from_template(task->output_filename,
										 task->output_template,
										 rte_lcore_id(),
										 task->output_count,
										 &task->output_tstamp);
					...
					// Reopen a file
					task->output_buffer =
						file_open_func(task->output_filename);
