...
/**
 * @brief Display global statistics in the specified window
 *
 * @param window Pointer to the window where the statistics will be displayed
...
/**
 * @brief Display capture statistics in the specified window
 *
 * @param window Pointer to the window where the statistics will be displayed
 * @param data Pointer to the stats_data structure containing the statistics data
 */
	...
	for (i = 0; i < data->port_list_size; i++)
	{
		...
		for (j = 0; j < data->queue_per_port; j++)
		{
			...
			wprintw(window, "         Ring: Enqueued: %s",
					ul_format(data->cores_stats_capture_list[i *
																 data->queue_per_port +
															 j]
								  .packets));
...
/**
 * @brief Writes the statistics to the provided window
 *
 * This function calculates aggregated and instant statistics from writing cores and writes the output to the given window.
 *
 * @param window Pointer to the ncurses window for displaying the statistics
 * @param data Pointer to the stats_data structure containing the statistics data
 */
static void wwrite_stats(WINDOW *window, struct stats_data *data)
{
	...
	instant_packets = (total_packets - last_total_packets) * 1000 / STATS_PERIOD_MS;
...
/**
 * @brief Signal handler function for catching and handling signals
 *
 * This function sets a flag indicating that the program should stop when a signal is caught.
 *
...
/**
 * @brief Moves and resizes the ncurses windows
 *
 * This function adjusts the size and position of the ncurses windows for displaying statistics.
 */
...
/**
 * @brief Initializes the ncurses windows for displaying statistics
 */
 ...
 *
 * This function updates and prints the statistics data onto the ncurses windows.
 *
 * @param timer Pointer to the rte_timer structure
 * @param data Pointer to the stats_data structure containing the statistics data
 * @return Always returns 0
 */
static int printscreen(__attribute__((unused)) struct rte_timer *timer, __attribute__((unused)) struct stats_data *data)
{
...
/**
 * @brief Starts the display of statistics and waits for user input
 *
 * This function initializes the ncurses environment, sets up windows, initializes timers, and waits for user input to stop the program.
 *
 * @param data Pointer to the stats_data structure containing the statistics data
 */
